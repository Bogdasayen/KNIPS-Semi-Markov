...
generate_model_outputs <- function(hesim_dat, model_inputs) {
  ...
  yrs_to_2nd_rev_combined <- rbindlist(yrs_to_2nd_rev_implant)
  ...
  # This takes too much memory on a desktop so don't run
  if(0) {
  ...
  # Add disutilities and costs for the 1st and 2nd revision
  
  # Disutilites and costs for 1st revision is added to 2nd state
  # Discount the costs and disutilities (continuous discounting but apply a floor() to use discrete)
  revision_disutilities_1st <- ((1 / (1 + discount_rate)) ^ yrs_to_1st_rev_combined)* rep(rep(revision_disutility, each = n_patients), n_strategies)
  revision_costs_1st <- ((1 / (1 + discount_rate)) ^ yrs_to_1st_rev_combined)* revision_cost
  revision_disutilities_2nd <- ((1 / (1 + discount_rate)) ^ yrs_to_2nd_rev_combined)* rep(rep(revision_disutility, each = n_patients), n_strategies)
  revision_costs_2nd <- ((1 / (1 + discount_rate)) ^ yrs_to_2nd_rev_combined)* revision_cost
  # Ensure they are zero if the event doesn't occur
  revision_disutilities_1st[yrs_to_1st_rev_combined == 0] <- 0
  revision_disutilities_2nd[yrs_to_2nd_rev_combined == 0] <- 0
  revision_costs_1st[yrs_to_1st_rev_combined == 0] <- 0
  revision_costs_2nd[yrs_to_2nd_rev_combined == 0] <- 0
  # Average over patients and strategies
  # Need indices for samples and strategies
  revision_disutilities_1st$sample <- 
    revision_disutilities_2nd$sample <- 
    revision_costs_1st$sample <- 
    revision_costs_2nd$sample <- 
    rep(rep(1:n_samples, each = n_patients), n_strategies)
  revision_disutilities_1st$strategy_id <- 
    revision_disutilities_2nd$strategy_id <- 
    revision_costs_1st$strategy_id <- 
    revision_costs_2nd$strategy_id <- 
    rep(1:n_strategies, each = n_patients * n_samples)
  
  revision_costs_1st_mean <- revision_costs_2nd_mean <- 
    revision_disutilities_1st_mean <- revision_disutilities_2nd_mean <-
    data.table(strategy_id = rep(strategies$strategy_id, each = n_samples),
                                           sample = rep(c(1:n_samples), n_strategies),
                                        mean = 0)
  # Calculate the average over all patients for each strategy and sample
  # Likely a way to do this without but the for loop doesn't take long
  for(strategy_i in 1:n_strategies) {
    revision_costs_1st_mean[strategy_id == strategy_i, "mean"] <- 
      revision_costs_1st[strategy_id == strategy_i, .(Mean = colMeans(.SD)), by = sample, .SDcols = c("yrs_to_1st_rev_simulated")][, "Mean"]
    revision_costs_2nd_mean[strategy_id == strategy_i, "mean"] <- 
      revision_costs_2nd[strategy_id == strategy_i, .(Mean = colMeans(.SD)), by = sample, .SDcols = c("yrs_to_2nd_rev_simulated")][, "Mean"]
    revision_disutilities_1st_mean[strategy_id == strategy_i, "mean"] <- 
      revision_disutilities_1st[strategy_id == strategy_i, .(Mean = colMeans(.SD)), by = sample, .SDcols = c("yrs_to_1st_rev_simulated")][, "Mean"]
    revision_disutilities_2nd_mean[strategy_id == strategy_i, "mean"] <- 
      revision_disutilities_2nd[strategy_id == strategy_i, .(Mean = colMeans(.SD)), by = sample, .SDcols = c("yrs_to_2nd_rev_simulated")][, "Mean"]
  }
  # Rearrange to match order of costs_ and qalys_ (ideally reorder everything at creation to match...)
  
  revision_costs_1st_mean <- revision_costs_1st_mean %>% arrange(sample, strategy_id)
  revision_costs_2nd_mean <- revision_costs_2nd_mean %>% arrange(sample, strategy_id)
  revision_disutilities_1st_mean <- revision_disutilities_1st_mean %>% arrange(sample, strategy_id)
  revision_disutilities_2nd_mean <- revision_disutilities_2nd_mean %>% arrange(sample, strategy_id)
  
  # Finally add these extra costs and disutilities
  economic_model$costs_[state_id == 2 & category == "Medical", "costs"] <- economic_model$costs_[state_id == 2 & category == "Medical", "costs"] +
    revision_costs_1st_mean[, "mean"]  
  economic_model$costs_[state_id == 3 & category == "Medical", "costs"] <- economic_model$costs_[state_id == 2 & category == "Medical", "costs"] +
    revision_costs_2nd_mean[, "mean"]  
  
  economic_model$qalys_[state_id == 2, "qalys"] <- economic_model$qalys_[state_id == 2, "qalys"] +
    revision_disutilities_1st_mean[, "mean"]
  economic_model$qalys_[state_id == 3, "qalys"] <- economic_model$qalys_[state_id == 3, "qalys"] +
    revision_disutilities_2nd_mean[, "mean"]
