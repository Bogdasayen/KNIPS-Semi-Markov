...
for(implant_name in implant_names) {
  total_costs[, implant_name] <-  as.matrix(ce_sim$costs[ce_sim$costs$category == "total" & 
                                                           ce_sim$costs$strategy_id == which(implant_names == implant_name), "costs"]) / n_patients
  total_qalys[, implant_name] <-  as.matrix(ce_sim$qalys[ce_sim$qalys$strategy_id == which(implant_names == implant_name), "qalys"]) / n_patients
}
