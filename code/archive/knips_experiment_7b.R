...
# V7b Set "Surgery Death" to be an absorbing death state rather than non-death health state
...
for(implant_name in implant_names) {
  ...
  for(i_sample in 1:n_samples) {
    ...
    transmod_params <- params_surv_list(
      ...
      params_surv(coefs = list(as.matrix(log(-log(1 - 
                                                    rbeta(n_samples_temp, 
                                                          shape1 = rerevision_mortality_90d_beta_params[1], 
                                                          shape2 = rerevision_mortality_90d_beta_params[2]) * (1 - exp(- exp(rnorm(n = n_samples_temp,
                                                                                                                                   mean = lograte_higher_revision_mean,
                                                                                                                                   sd = lograte_higher_revision_se))))
                                                  ))),
                               as.matrix(log(-log(1 - rbeta(n_samples_temp, 
                                                            shape1 = revision_mortality_90d_beta_params[1], 
                                                            shape2 = revision_mortality_90d_beta_params[2]))))
                               ),
