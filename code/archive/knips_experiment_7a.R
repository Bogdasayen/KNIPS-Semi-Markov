...
# ERROR Line 781: Still not adding to 1: econmod$stateprobs_[econmod$stateprobs_$strategy_id ==1 & econmod$stateprobs_$sample ==1 & econmod$stateprobs_$t ==30, ]
...
# V6 uses PROMS utilities and corrects mortality to be clock-forward
# V7 adds "Surgery Death" state with rates informed by 90-day mortality from NJR
# V7a tried changing "Surgery Death" to a non-absorbing state with some rate of going to "Death"
...
states <- data.table( # Non-other cause death health states
  state_id = 1:4,
  state_name = state_names[1:4]#, "Surgery Death")
) 
...
# Only 9 (first revision, 2nd revision, 3 background mortality death rates, 3 surgery death rates, 1 from surgery death to death)
tmat <- rbind(c(NA, 1, NA, 6, 2), # From "Post TKR"
              c(NA, NA, 3, 7, 4), # From "Post 1st revision"
              c(NA, NA, NA, 8, 5), # From "Post 2nd revision"
              c(NA, NA, NA, NA, 9), # From "Surgery Death" to "Death" (to avoid absorbing state)
...
# Transition 2
# Post TKR to death  (background all-cause mortality, not 90d post surgery mortality)
...
mortality_row_index <- which(mortality_90d_raw[, "age_lower"] == age_range[1] & 
                               mortality_90d_raw[, "age_upper"] == age_range[2] &
...
# Zero after 90 days but should depend on probability higher revision and 90d mortality
rerevision_mortality_90d <- cbind(rerevision_mortality_90d, 0)
...
for(implant_name in implant_names) {
  ...
  for(i_sample in 1:n_samples) {
    ...
    transmod_params <- params_surv_list(
      ...
      params_surv(coefs = list(rate = as.matrix(log(-log(1 - rbeta(n_samples_temp, 
                                              shape1 = tkr_mortality_90d_beta_params[1], 
                                              shape2 = tkr_mortality_90d_beta_params[2])))),
                     as.matrix(rep(-Inf, n_samples_temp))),
        aux = list(time = tkr_mortality_times_90d),
        dist = "pwexp"),
      
      # 7. Post 1st revision to surgery death
      params_surv(coefs = list(as.matrix(log(-log(1 - rbeta(n_samples_temp, 
                                              shape1 = revision_mortality_90d_beta_params[1], 
                                              shape2 = revision_mortality_90d_beta_params[2])))),
                     as.matrix(rep(-Inf, n_samples_temp))),
        aux = list(time = revision_mortality_times_90d),
        dist = "pwexp"),
      
      # 8. Post 2nd revision to surgery death
      # Need to update so that it depends on ongoing risk of revision
      params_surv(coefs = list(as.matrix(log(-log(1 - rbeta(n_samples_temp, 
                                                        shape1 = rerevision_mortality_90d_beta_params[1], 
                                                      shape2 = rerevision_mortality_90d_beta_params[2])))),
                               as.matrix(rep(-Inf, n_samples_temp))),
                  aux = list(time = rerevision_mortality_times_90d),
                  dist = "pwexp"),
      
      # 9. Surgery Death to Death
      # Only needed to avoid coding error with multiple absorbing states
      params_surv(coefs = lapply(as.list(log(mr)), matrixv),
                  aux = list(time = c(0:time_horizon)),
                  dist = "pwexp")
    ...
    #x <-  transmod$sim_stateprobs(t = 0:(time_horizon))
    # with(x, prob[strategy_id ==1 & sample == 1 & t==35])
...
utility_row_index <- which(utilities_raw[, "age_lower"] == age_range[1] & 
                             utilities_raw[, "age_upper"] == age_range[2] &
...
with(econmod$stateprobs_, mean(prob[strategy_id == 1 & state_id == 1]))
# Average in post 1st revision
with(econmod$stateprobs_, mean(prob[strategy_id == 1 & state_id == 2]))
# Average in post 2nd revision
with(econmod$stateprobs_, mean(prob[strategy_id == 1 & state_id == 3]))
# Average probability of occupying surgery death
with(econmod$stateprobs_, mean(prob[strategy_id == 1 & state_id == 4]))
# Average probability of occupying death
with(econmod$stateprobs_, mean(prob[strategy_id == 1 & state_id == 5]))
